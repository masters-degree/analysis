...
if __name__ == '__main__':
    with getConnection() as connection:
        ...
        for mark, count in marksCountSorted:
            if mark <= leftBorder:
                leftBorderCount.append(count)

        for mark, count in marksCountSorted:
            if mark >= rightBorder:
                rightBorderCount.append(count)

        biggestLeftMark = []
        biggestRightMark = []

        print(f"Левая граница максимального количества оценок {np.mean(leftBorderCount) * 1.5}")
        print(f"Правая граница максимального количества оценок {np.mean(rightBorderCount) * 1.5}")
        ...
        for mark, count in marksCountSorted:
            if mark <= leftBorder:
                if count > (np.mean(leftBorderCount) * 1.5) and count > countM:
                    biggestLeftMark.append(mark)
            if mark >= rightBorder:
                if count > (np.mean(rightBorderCount) * 1.5) and count > countM:
                    biggestRightMark.append(mark)

        for mark in [*biggestLeftMark, *biggestRightMark]:
            print(f"Удаляется оценка {mark} с количеством {marksCount[mark]}")
            del marksCount[mark]
